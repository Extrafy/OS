...
int sys_ipc_try_send(u_int envid, u_int value, u_int srcva, u_int perm) {
	...
	if (srcva != 0) {
		...
		if(p == NULL || is_illegal_va(e->env_ipc_dstva)) return -E_INVAL;
...
int sys_write_dev(u_int va, u_int pa, u_int len) {
	...
	if(!(pa >= 0x180003f8 && pa + len <= 0x18000418) && !(pa >= 0x180001F0 && pa + len <= 0x180001F8)) return -E_INVAL;
	memcpy((void*)(0xa0000000 + pa), va, len);
	return 0;
}
...
int sys_read_dev(u_int va, u_int pa, u_int len) {
	...
	if(len != 1 && len != 2 && len != 4) return -E_INVAL;
        if(is_illegal_va_range(va, len)) return -E_INVAL;
	if(!(pa >= 0x180003f8 && pa + len <= 0x18000418) && !(pa >= 0x180001F0 && pa + len <= 0x180001F8)) return -E_INVAL;
        memcpy(va, (void*)(0xa0000000 + pa), len);
	return 0;
}
