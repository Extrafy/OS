...
int copy_file_content(struct File *src, struct File *dst) {
   ...
   for (u_int i = 0; i < nblock; i++) {
      ...
      struct File *files1 = (struct File *)src_blk;
      struct File *files2 = (struct File *)dst_blk;
      struct File *f1;
      struct File *f2;
      for (f1 = files1, f2 = files2; f1 < files1 + FILE2BLK; ++f1, ++f2) {
...
int copy_directory_contents(struct File *src, struct File *dst) {
   ...
   for (u_int i = 0; i < ROUND(src->f_size, BLOCK_SIZE) / BLOCK_SIZE; i++) {
	 ...
	 if(dst_file->f_type == FTYPE_REG) copy_file_content(dir_content + j, dst_file);
	 else copy_directory_contents(dir_content + j , dst_file);
      }
  //    file_dirty(dst, i * BLOCK_SIZE);
   }
   file_flush(dst);
